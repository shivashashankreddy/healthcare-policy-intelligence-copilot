...
Built a local-first Healthcare Claims and Policy RAG Copilot with FastAPI, LangChain, Chroma, synthetic policy documents, role-aware query modes, citation-grounded retrieval, hallucination guardrails, sanitized JSONL audit logging, lightweight RAG evaluation, Docker, Pytest, and GitHub Actions CI.

## Interview Talking Points

- Why healthcare RAG systems need citations, confidence signals, auditability, and cautious failure modes.
- How role-aware query modes map to real operations teams without creating separate applications for every workflow.
- Why synthetic data boundaries are essential for healthcare portfolio projects and interview demos.
- How retrieval confidence, citation presence, and policy ambiguity drive guardrail behavior.
- How the local deterministic generation layer can be replaced with a production LLM adapter while preserving retrieval, guardrails, and audit controls.
- How JSONL audit logs support reviewability while reducing sensitive data retention risk.
